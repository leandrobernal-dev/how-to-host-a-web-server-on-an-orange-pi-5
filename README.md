...
We will use docker to easily create and manage our server and database. Furthermore we will be setting up and use Cloudflare's *free* tunnel system to safely expose your server on the internet.
...
3. Performance: Specwise it is significantly much faster than the latest version of raspberry pi which is the rpi 4. And with it's m.2 slot it enables us to run an os much faster than the rpi4 can.
4. Availability: The past few years has been tough for rpi manufacturer to serve the demand of the rpi 4 leading it's prices to go up.<br/><br/>
